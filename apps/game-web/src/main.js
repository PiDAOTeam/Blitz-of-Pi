...
function Ga(e = "", t = "") {
  const r = { recharge: "\u5145\u503C", reward: "\u83B7\u5956", battle_entry: "\u5165\u573A\u8D39", battle_refund: "\u9000\u56DE", withdraw_lock: "\u63D0\u73B0\u51BB\u7ED3", withdraw_unlock: "\u63D0\u73B0\u9000\u56DE", withdraw_complete: "\u63D0\u73B0\u5B8C\u6210", withdraw_reject: "\u63D0\u73B0\u9A73\u56DE", transfer_out: "\u8F6C\u8D26\u652F\u51FA", transfer_in: "\u8F6C\u8D26\u6536\u5165", transfer_fee: "\u8F6C\u8D26\u624B\u7EED\u8D39", invite_reward: "\u9080\u8BF7\u5956\u52B1", invite_commission: "\u9080\u8BF7\u63D0\u6210" };
...
function vr(e) {
  ...
  return t.length ? `<section class="wallet-ledger-panel">
    ...
    <div class="wallet-ledger-list">
      ${u.map((p) => {
    const f = Number(p.amount || 0), m = Number(p.balance_after || 0), g = kr(p.direction), P = p.direction === "in" || p.direction === "unlock" ? "in" : p.direction === "lock" ? "lock" : "out", C = p.created_at ? new Date(p.created_at).toLocaleString(void 0, { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" }) : "-";
    return `<article class="wallet-ledger-item ${P}">
            <div>
              <strong>${i(Ga(p.type, p.direction))}</strong>
              <small>${i(p.remark || C)}</small>
            </div>
            <div>
              <b>${g}${f.toFixed(4)} Pi</b>
              <small>${i(s)} ${m.toFixed(4)}</small>
            </div>
          </article>`;
  }).join("")}
    </div>
...
function _() {
  ...
  `, document.querySelector("#start-match")?.addEventListener("click", async () => {
    Xa();
  }), Dr(), document.querySelector("#open-home-invite")?.addEventListener("click", () => at()), document.querySelector("#open-game-tips")?.addEventListener("click", () => Hr()), document.querySelector("#open-rank-leaderboard")?.addEventListener("click", async () => {
    await Kt(1), Ot();
  }), da();
}
function Ot() {
...
function L() {
  ...
  `, document.querySelector("#edit-profile")?.addEventListener("click", () => Qa()), document.querySelector("#open-rank-rules")?.addEventListener("click", () => zr()), document.querySelector("#toggle-wallet-ledger")?.addEventListener("click", () => {
    ...
    const r = Math.max(1, Math.ceil((a.wallet?.ledgers?.length || 0) / fe));
...
function Ye(e, t) {
  ...
  if (o.width <= 0 || o.height <= 0) return He(e, t);
  const s = window.getComputedStyle(r), l = Number.parseFloat(s.paddingLeft) || 0, c = Number.parseFloat(s.paddingRight) || 0, d = Number.parseFloat(s.paddingTop) || 0, u = Number.parseFloat(s.paddingBottom) || 0, h = o.left + l, p = o.top + d, f = o.width - l - c, m = o.height - d - u;
  if (f <= 0 || m <= 0) return He(e, t);
  const g = f / Pe, P = m / $e, C = g * Pa, T = P * Pa;
  if (e < h - C || e > h + f + C || t < p - T || t > p + m + T) return He(e, t);
  const U = Math.min(Pe - 1, Math.max(0, Math.floor((e - h) / g)));
  return { row: Math.min($e - 1, Math.max(0, Math.floor((t - p) / P))), col: U };
}
function He(e, t) {
...
function Hi() {
  ...
  Ln(), (f || !document.querySelector("#game-board")) && (Pt = t.roomNo, Se = null, Re = "", je = "", se = Je(o.board || []), Ve = "", Ke = "", K = "", R.innerHTML = `
    <main class="battle-shell ${t.remainSeconds <= 10 && !l ? "battle-final-countdown" : ""}">
      ...
      <section class="board-panel">
        <div class="game-board${Ta()}" id="game-board">
          ${Ut(o)}
        </div>
      </section>
  ...
  if (U !== Re) {
    ...
    if (E) {
      E.className = `game-board${Ta()}`, oi(E, o, H && ue && !l);
...
function sn(e = n("roomGoneAlert")) {
  Q(), O(), ce(), a.screen = "home", a.roomNo = "", a.roomJoinToken = "", a.room = null, a.realtimeRoom = null, a.selectedTile = null, a.tileEffect = null, a.networkStatus = "connecting", a.networkLatencyMs = 0, a.vsIntroUntil = 0, a.lastRoomStateAt = 0, a.lastSwapSentAt = 0, a.pendingSwapSeq = 0, a.pendingSwapPositions = [], a.lastSwapPositions = [], a.battleConnectingAt = 0, a.battleEnteredAt = 0, a.feedback = null, a.feedbackEventId = "", a.battleMessage = "", a.matchCancelMessage = "", _(), window.setTimeout(() => {
    S(e, "error");
  }, 80);
}
function ln(e = true) {
  ...
  Q(), e && (he = 0), a.battleConnectingAt = Date.now(), a.battleEnteredAt = 0, a.feedback = null, a.feedbackEventId = "", a.networkStatus = "connecting", a.networkLatencyMs = 0, a.lastRoomStateAt = 0, a.lastSwapSentAt = 0, a.pendingSwapSeq = 0, a.pendingSwapPositions = [], a.lastSwapPositions = [];
  const r = new URL(Ia, window.location.href);
...
async function D() {
  const [e, t, r, o, s, l, c, d, u, h, p] = await Promise.all([w("/api/home/index"), w("/api/auth/profile"), w("/api/wallet/me"), w("/api/rank/me"), w("/api/rank/leaderboard?page=1&pageSize=15&type=weekly"), w("/api/pi/config"), w("/api/profile/options"), w("/api/game/config"), w(`/api/battle/history?page=${a.battleHistoryPage}&pageSize=5&mode=${encodeURIComponent(a.battleHistoryFilter === "all" ? "" : a.battleHistoryFilter)}`), w("/api/withdraw/wallets").catch(() => []), w("/api/invite/me").catch(() => null)]);
  a.home = e, a.user = t, a.wallet = r, a.rankStatus = o, a.rankLeaderboard = s, a.piConfig = l, a.profileOptions = c, a.gameConfig = d, a.inviteInfo = p, Qe(), a.withdrawWallets = h, a.battleHistory = u.items, a.battleHistoryPage = u.page, a.battleHistoryTotal = u.total, a.battleHistoryTotalPages = u.totalPages;
}
async function vt(e) {
