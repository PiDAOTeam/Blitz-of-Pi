...
function it({ admin: e, config: t, piConfig: a, gameConfig: n, dashboard: s, rooms: r, paymentOrders: o, users: m, ledgers: R, battleRooms: y, withdrawOrders: x, reconciliation: j, riskAudit: A, auditLogs: B, rankStarRecords: w, rankChestRecords: L, rankLeaderboard: T, rankWeeklySettlements: C, growthOps: M, watchShareholderOps: watchOps, banner: U }) {
  ...
  const extremeRealtimeModeSummary = "\u5FEB\u901F\u5F00\u6218 / \u5C0F\u5BCC\u8C6A / \u5927\u5BCC\u8C6A / \u8D85\u7EA7\u5BCC\u8C6A";
  const extremeRealtimeStatusText = "\u5F53\u524D\uFF1A\u6781\u9650\u624B\u611F\u5DF2\u542F\u7528";
  const extremeRealtimeStatusClass = "success";
      ...
      <section class="overview-columns">
        ...
        <section class="panel overview-block">
          ...
          <div class="mini-grid">
            ...
            ${f("\u673A\u5668\u4EBA", s.recentBotFinishedCount ?? 0, s.botStatus === "ok" ? `\u8FD1\u4E24\u5C0F\u65F6\u5DF2\u5B8C\u6210 \u00B7 \u4ECA\u65E5 ${s.todayBotFinishedCount ?? 0} \u5C40` : "\u8FD1\u4E24\u5C0F\u65F6\u6682\u65E0\u5B8C\u6210\u5C40\uFF0C\u82E5\u591C\u665A\u4EBA\u5C11\u53EF\u80FD\u662F\u7A7A\u95F2")}
            ${f("\u51BB\u7ED3\u95E8\u7968", s.frozenPaidRoomCount ?? 0, Number(s.frozenPaidRoomCount || 0) ? "\u542B\u6B63\u5728\u6253\u7684\u4ED8\u8D39\u5C40\uFF1B\u8D85\u8FC7\u51C6\u5907\u7A97\u4E14\u6CA1\u8FDB\u623F\u4F1A\u81EA\u52A8\u9000\u7968" : "\u6CA1\u6709\u5361\u6B7B\u7684\u4ED8\u8D39\u7968")}
            ${f("\u4ECA\u65E5\u81EA\u52A8\u9000\u7968", s.todayReleasedDrawCount ?? 0, "\u6D41\u5C40/\u672A\u8FDB\u623F\u5DF2\u89E3\u51BB\u95E8\u7968")}
  ...
  if (_ === "config") return `
      ...
      <section class="panel">
        ...
        <form id="game-config-form" class="form">
          ...
          <label><span>\u6307\u5B9A\u5F00\u653E Pi UID</span><textarea name="assetGatewayGrayUids" rows="2" placeholder="\u7559\u7A7A\u8868\u793A\u5168\u90E8\u7528\u6237\uFF0C\u591A\u4E2A\u6362\u884C">${(n.assetGateway?.grayUserPiUids || []).join(`
`)}</textarea></label>
          <label><span>\u6307\u5B9A\u5F00\u653E username</span><textarea name="assetGatewayGrayUsernames" rows="2" placeholder="\u53EF\u9009\uFF0C\u4EC5\u8F85\u52A9\u5339\u914D">${(n.assetGateway?.grayUserPiUsernames || []).join(`
`)}</textarea></label>
          <p class="meta">\u5F00\u542F\u8D44\u4EA7\u540C\u6B65\u540E\uFF0C\u524D\u53F0\u624D\u4F1A\u663E\u793A\u79EF\u5206/POC\u5E76\u5141\u8BB8\u5165\u573A\u3002\u6307\u5B9A\u540D\u5355\u7559\u7A7A\u8868\u793A\u5168\u90E8\u7528\u6237\u5F00\u653E\uFF0C\u586B\u5199 UID/username \u8868\u793A\u53EA\u7ED9\u540D\u5355\u7528\u6237\u5F00\u653E\u3002\u79EF\u5206/POC\u9ED8\u8BA4\u53EA\u771F\u4EBA\u5339\u914D\uFF1B\u79EF\u5206\u95E8\u7968\u5FC5\u987B\u662F\u6574\u6570\uFF0C\u4E0D\u80FD\u586B\u5C0F\u6570\u3002</p>
          <hr />
          <h3 class="sub-panel-title">HashPi APP \u63A5\u5165</h3>
          <label>
            <span>APP \u6865\u63A5\u767B\u5F55</span>
            <select name="hashPiAppBridgeEnabled">
              <option value="true" ${n.hashPiAppBridge?.enabled !== false ? "selected" : ""}>\u5F00\u542F</option>
              <option value="false" ${n.hashPiAppBridge?.enabled === false ? "selected" : ""}>\u5173\u95ED</option>
            </select>
          </label>
          <div class="mode-checkboxes">
            ${["quick_battle", "points_battle", "poc_battle", "pi_battle"].map((mode) => `<label><input type="checkbox" name="hashPiAppBridgeMode" value="${mode}" ${(n.hashPiAppBridge?.allowedModes || ["quick_battle", "points_battle", "poc_battle", "pi_battle"]).includes(mode) ? "checked" : ""} /> ${i(oe(mode))}</label>`).join("")}
          </div>
          <label><span>\u7968\u636E\u6709\u6548\u671F\uFF08\u79D2\uFF09</span><input name="hashPiAppBridgeTicketTtlSeconds" type="number" inputmode="numeric" value="${n.hashPiAppBridge?.ticketTtlSeconds ?? 120}" disabled /></label>
          <label><span>\u767B\u5F55\u5931\u8D25\u63D0\u793A</span><input name="hashPiAppBridgeFailureMessage" maxlength="120" value="${i(plainConfigText(n.hashPiAppBridge?.failureMessage, "HashPi APP \u767B\u5F55\u5931\u8D25\uFF0C\u8BF7\u8FD4\u56DE HashPi \u540E\u91CD\u8BD5\u3002"))}" /></label>
          <label><span>Pi \u5145\u503C\u5F15\u5BFC</span><input name="hashPiAppBridgePiRechargeHint" maxlength="160" value="${i(plainConfigText(n.hashPiAppBridge?.piRechargeHint, "APP \u5185\u6682\u4E0D\u652F\u6301 Pi \u5145\u503C\uFF0C\u8BF7\u7528 Pi Browser \u6253\u5F00 Pi\u95EA\u7535\u6218\u5B8C\u6210\u5145\u503C\u3002"))}" /></label>
          <p class="meta">HashPi \u9996\u9875\u6309\u94AE\u94FE\u63A5\u586B\uFF1Ahttps://hashpi.app/blitz-bridge/\u3002\u7968\u636E\u7531 HashPi \u540E\u7AEF\u4E00\u6B21\u6027\u7B7E\u53D1\uFF0C\u5F53\u524D 120 \u79D2\u6709\u6548\uFF1BBlitz \u53EA\u6D88\u8D39\u6821\u9A8C\uFF0C\u4E0D\u4FE1\u4EFB\u524D\u7AEF\u4F20\u6765\u7684\u7528\u6237\u8D44\u6599\u3002</p>
          <hr />
          <label>
            <span>\u5C0F\u5BCC\u8C6A\uFF08\u79EF\u5206\uFF09\u5F00\u5173</span>
          ...
          <div class="ops-config-section bot-config-section">
            <div class="ops-config-header">
              <div>
                <h3>\u667A\u80FD\u8865\u4F4D\u673A\u5668\u4EBA</h3>
                <p>\u5C0F\u5BCC\u8C6A\u771F\u4EBA\u4E0D\u8DB3\u65F6\u81EA\u52A8\u8865\u4F4D\uFF0C\u63D0\u9AD8\u5339\u914D\u6210\u529F\u7387\u3002</p>
              </div>
              <strong class="${n.pointsBattle?.botMatchEnabled ? "ops-state-on" : "ops-state-off"}">${n.pointsBattle?.botMatchEnabled ? "\u5DF2\u5F00\u542F" : "\u5DF2\u5173\u95ED"}</strong>
            </div>
            <div class="ops-config-grid">
              <label><span>\u5C0F\u5BCC\u8C6A\u667A\u80FD\u8865\u4F4D</span><select name="pointsBotMatchEnabled"><option value="true" ${n.pointsBattle?.botMatchEnabled ? "selected" : ""}>\u5F00\u542F</option><option value="false" ${n.pointsBattle?.botMatchEnabled ? "" : "selected"}>\u5173\u95ED</option></select></label>
              <label><span>\u8865\u4F4D\u7B49\u5F85\u79D2\u6570</span><input name="pointsBotFallbackSeconds" type="number" inputmode="numeric" min="5" max="120" step="1" value="${n.pointsBattle?.botFallbackSeconds ?? 15}" /></label>
              <label><span>\u8865\u4F4D\u96BE\u5EA6</span><select name="pointsBotDifficulty"><option value="easy" ${n.pointsBattle?.botDifficulty === "easy" ? "selected" : ""}>\u7B80\u5355</option><option value="normal" ${!n.pointsBattle?.botDifficulty || n.pointsBattle?.botDifficulty === "normal" ? "selected" : ""}>\u666E\u901A</option><option value="hard" ${n.pointsBattle?.botDifficulty === "hard" ? "selected" : ""}>\u56F0\u96BE</option><option value="expert" ${n.pointsBattle?.botDifficulty === "expert" ? "selected" : ""}>\u9AD8\u624B</option></select></label>
              <label><span>\u8865\u4F4D\u5C40\u80DC\u5229\u5956\u52B1</span><select name="pointsBotRewardsEnabled"><option value="true" ${n.pointsBattle?.botRewardsEnabled ? "selected" : ""}>\u53D1\u653E</option><option value="false" ${n.pointsBattle?.botRewardsEnabled ? "" : "selected"}>\u4E0D\u53D1\u653E</option></select></label>
              <label><span>\u8BA1\u5165\u6392\u884C/\u5468\u699C</span><select name="pointsBotCountInRank"><option value="true" ${n.pointsBattle?.botCountInRank !== false ? "selected" : ""}>\u8BA1\u5165</option><option value="false" ${n.pointsBattle?.botCountInRank === false ? "selected" : ""}>\u4E0D\u8BA1</option></select></label>
              <label><span>\u8BA1\u5165\u8155\u8868\u5206\u7EA2</span><select name="pointsBotCountInWatchShareholder"><option value="true" ${n.pointsBattle?.botCountInWatchShareholder !== false ? "selected" : ""}>\u8BA1\u5165</option><option value="false" ${n.pointsBattle?.botCountInWatchShareholder === false ? "selected" : ""}>\u4E0D\u8BA1</option></select></label>
              <label class="ops-config-wide"><span>\u673A\u5668\u4EBA\u540D\u79F0\u6C60</span><textarea name="pointsBotNamePool" rows="6" placeholder="\u6BCF\u884C\u4E00\u4E2A\u540D\u79F0\uFF0C\u7CFB\u7EDF\u968F\u673A\u4F7F\u7528">${(n.pointsBattle?.botNamePool || []).join("\n")}</textarea></label>
            </div>
            <p class="meta">\u5EFA\u8BAE\uFF1A\u5F00\u542F\uFF0C\u7B49\u5F85 15 \u79D2\uFF0C\u96BE\u5EA6\u9009\u666E\u901A\u3002\u540D\u79F0\u6C60\u6700\u591A 200 \u4E2A\uFF0C\u8865\u4F4D\u5C40\u4F1A\u7559\u4E0B\u540E\u53F0\u6807\u8BB0\uFF0C\u4FBF\u4E8E\u5BF9\u8D26\u548C\u98CE\u63A7\u3002</p>
          </div>
          <hr />
          ...
          <div class="ops-state-box ${extremeRealtimeStatusClass}">
            ...
            <span>\u751F\u6548\u573A\u6B21\uFF1A${extremeRealtimeModeSummary}</span>
          </div>
          <label><span>\u6700\u5927\u5F85\u786E\u8BA4\u6B65\u6570</span><input name="extremeRealtimeMaxPendingSwaps" type="number" inputmode="decimal" min="1" max="6" step="1" value="${n.extremeRealtime?.maxPendingSwaps ?? 3}" /></label>
          ...
          <p class="meta">\u56DB\u4E2A\u573A\u6B21\u7EDF\u4E00\u542F\u7528\u65B0\u624B\u611F\u3002\u4E0D\u61C2\u65F6\u4FDD\u6301\u9ED8\u8BA4\uFF1A3\u6B65 / 2000\u6BEB\u79D2 / 120\u6BEB\u79D2 / 0.05\u3002</p>
          <hr />
          <h3 class="sub-panel-title">\u753B\u9762\u4E0E\u6027\u80FD</h3>
          <p class="meta">\u524D\u53F0\u5DF2\u7EDF\u4E00\u5747\u8861\uFF1A\u73A9\u5BB6\u4E0D\u518D\u80FD\u5207\u70AB\u5F69\uFF1B\u7279\u6548\u4FDD\u7559\uFF0C\u52A8\u4F5C\u66F4\u987A\u3002\u5F31\u673A\u4ECD\u4F1A\u81EA\u52A8\u964D\u7279\u6548\u4FDD\u6D41\u7545\u3002</p>
          <label>
            <span>\u9ED8\u8BA4\u6548\u679C\u6A21\u5F0F</span>
            <select name="effectDefaultMode">
              <option value="balanced" selected>\u5747\u8861\u6A21\u5F0F</option>
            </select>
          </label>
          <label>
          ...
          <label>
            ...
            <select name="effectAutoDowngradeEnabled">
              <option value="true" ${n.visualEffects?.autoDowngradeEnabled !== false ? "selected" : ""}>\u5F00\u542F\uFF0C\u4F18\u5148\u4FDD\u6D41\u7545</option>
              <option value="false" ${n.visualEffects?.autoDowngradeEnabled === false ? "selected" : ""}>\u5173\u95ED\uFF0C\u59CB\u7EC8\u6309\u5F53\u524D\u7279\u6548</option>
            </select>
          </label>
          <label>
            <span>\u6ED1\u52A8\u62D6\u5C3E</span>
          ...
          <label>
            <span>\u5BF9\u6218\u8F7B\u91CF\u97F3\u6548</span>
            <select name="effectSoundEnabled">
              <option value="true" ${n.visualEffects?.soundEnabled !== false ? "selected" : ""}>\u5F00\u542F</option>
          ...
          <label>
            ...
            <select name="effectBgmEnabled">
              <option value="true" ${n.visualEffects?.bgmEnabled !== false ? "selected" : ""}>\u5F00\u542F</option>
          ...
          <div class="avatar-config">
            ...
            <div class="rank-row">
              <label><span>\u672C\u5730\u5927\u5B57\u63D0\u793A</span><input name="effectLocalBurstSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.localBurstSeconds ?? 1.18}" /></label>
              ...
              <label><span>\u6821\u5BF9\u5907\u7528\u63D0\u793A</span><input name="effectServerBurstSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.serverBurstSeconds ?? 1.22}" /></label>
            ...
            <div class="rank-row">
              ...
              <label><span>\u670D\u52A1\u7AEF\u6821\u5BF9</span><input name="effectServerSettleSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.serverSettleSeconds ?? 0.16}" /></label>
              <label><span>\u5F97\u5206\u95EA\u52A8</span><input name="effectImpactSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.impactSeconds ?? 0.82}" /></label>
            ...
            <div class="rank-row">
              <label><span>\u68CB\u76D8\u53CD\u9988</span><input name="effectBoardEffectSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.boardEffectSeconds ?? 0.28}" /></label>
              <label><span>\u70AB\u5F69\u68CB\u76D8</span><input name="effectBoardEffectHighSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.boardEffectHighSeconds ?? 0.34}" /></label>
              ...
              <label><span>\u70AB\u5F69\u65B9\u5757\u7206\u5F00</span><input name="effectTileBurstHighSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.tileBurstHighSeconds ?? 0.38}" /></label>
            </div>
            <div class="rank-row">
              <label><span>\u65B9\u5757\u4E0B\u843D</span><input name="effectTileFallSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.tileFallSeconds ?? 0.26}" /></label>
              <label><span>\u70AB\u5F69\u65B9\u5757\u4E0B\u843D</span><input name="effectTileFallHighSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.tileFallHighSeconds ?? 0.28}" /></label>
            ...
            <div class="rank-row">
              ...
              <label><span>\u65E0\u6548\u4EA4\u6362</span><input name="effectInvalidSwapSeconds" type="number" inputmode="decimal" min="0.05" max="3" step="0.01" value="${n.visualEffects?.animationDurations?.invalidSwapSeconds ?? 0.2}" /></label>
      ...
      <section class="panel match-ops-panel">
        <div class="section-head">
          <div>
            ...
            <h2>\u5F02\u5E38\u5BF9\u5C40\u5904\u7406\u4E2D\u5FC3</h2>
            <p class="meta">\u514D\u8D39\u673A\u5668\u4EBA\u5F02\u5E38\u5C40\u53EF\u76F4\u63A5\u4F5C\u5E9F\u3002\u4ED8\u8D39\u5C40\u5339\u914D\u540E\u4E00\u76F4\u6CA1\u4EBA\u8FDB\u623F\uFF0C\u7CFB\u7EDF\u4F1A\u6309\u51C6\u5907\u8D85\u65F6\u81EA\u52A8\u9000\u7968\uFF1B\u5DF2\u5F00\u6253\u7684\u4ED8\u8D39/\u771F\u4EBA\u5F02\u5E38\u5C40\u4ECD\u53EA\u8F6C\u4EBA\u5DE5\u590D\u6838\u3002</p>
          </div>
...
function inviteRequiredModeChecks(e = []) {
  const t = Array.isArray(e) && e.length ? e : ENGAGEMENT_BATTLE_MODES.map((a) => a.key);
  return `
    <div class="engagement-mode-checks">
      ${ENGAGEMENT_BATTLE_MODES.map((a) => `
        <label>
          <input type="checkbox" name="inviteBindRequiredMode" value="${a.key}" ${t.includes(a.key) ? "checked" : ""} />
          <span>${i(a.label)}</span>
        </label>
      `).join("")}
    </div>
    ...
    <section class="panel">
      ...
      <form id="growth-config-form" class="form">
        ...
        <div class="growth-config-grid">
          ...
          <label><span>\u672A\u7ED1\u5B9A\u9650\u5236</span><select name="inviteBindRequiredEnabled">
            <option value="true" ${n.bindRequiredEnabled !== false ? "selected" : ""}>\u5F00\u542F</option>
            <option value="false" ${n.bindRequiredEnabled === false ? "selected" : ""}>\u5173\u95ED</option>
          </select></label>
          <label><span>\u4F53\u9A8C\u5C40\u6570</span><input name="inviteBindRequiredAfterBattles" type="number" inputmode="decimal" min="0" max="100" step="1" value="${n.bindRequiredAfterBattles ?? 5}" /></label>
          <label><span>\u5B98\u65B9\u63A8\u8350\u4EBA</span><input name="officialInviterPiUsername" value="${i(n.officialInviterPiUsername || "")}" placeholder="Pi \u7528\u6237\u540D" /></label>
          <label><span>\u9650\u5236\u63D0\u793A</span><input name="inviteBindRequiredMessage" value="${i(n.bindRequiredMessage || "\u8BF7\u5148\u7ED1\u5B9A\u9080\u8BF7\u4EBA\uFF0C\u518D\u7EE7\u7EED\u5BF9\u6218\u3002")}" /></label>
...
function Ut(e, t, a) {
  ...
  y?.addEventListener("submit", async (c) => {
    ...
    x && (x.textContent = "\u4FDD\u5B58\u4E2D...");
    try {
      await d("/admin-api/game-config", { method: "POST", body: JSON.stringify(U({ quickBattle: { enabled: String(l.get("quickEnabled")) === "true", entryFee: 0, platformFeeRate: 0, rewardRate: 0, botMatchEnabled: String(l.get("quickBotMatchEnabled")) === "true", botRewardsEnabled: String(l.get("quickBotRewardsEnabled")) === "true" }, pointsBattle: { enabled: String(l.get("pointsEnabled")) === "true", entryFee: Number(l.get("pointsEntryFee") || 0), platformFeeRate: Number(l.get("pointsPlatformFeeRate") || 0), rewardRate: Number(l.get("pointsRewardRate") || 0), assetType: "POINTS", botMatchEnabled: String(l.get("pointsBotMatchEnabled")) === "true", botRewardsEnabled: String(l.get("pointsBotRewardsEnabled")) === "true", botFallbackSeconds: Number(l.get("pointsBotFallbackSeconds") || 15), botDifficulty: String(l.get("pointsBotDifficulty") || "normal"), botCountInRank: String(l.get("pointsBotCountInRank")) !== "false", botCountInWeekly: String(l.get("pointsBotCountInRank")) !== "false", botCountInWatchShareholder: String(l.get("pointsBotCountInWatchShareholder")) !== "false", botNamePool: String(l.get("pointsBotNamePool") || "").split(/\r?\n|,/).map((p) => p.trim()).filter(Boolean) }, pocBattle: { enabled: String(l.get("pocEnabled")) === "true", entryFee: Number(l.get("pocEntryFee") || 0), platformFeeRate: Number(l.get("pocPlatformFeeRate") || 0), rewardRate: Number(l.get("pocRewardRate") || 0), assetType: "POC", botMatchEnabled: false, botRewardsEnabled: false }, piBattle: { enabled: String(l.get("piBattleEnabled")) === "true", entryFee: Number(l.get("piBattleEntryFee") || 0), platformFeeRate: Number(l.get("piBattlePlatformFeeRate") || 0), rewardRate: Number(l.get("piBattleRewardRate") || 0), assetType: "PI", botMatchEnabled: false, botRewardsEnabled: false }, assetGateway: { enabled: String(l.get("assetGatewayEnabled")) === "true", pointsEnabled: String(l.get("assetGatewayPointsEnabled")) === "true", pocEnabled: String(l.get("assetGatewayPocEnabled")) === "true", summaryEnabled: true, grayUserPiUids: String(l.get("assetGatewayGrayUids") || "").split(/\r?\n|,/).map((p) => p.trim()).filter(Boolean), grayUserPiUsernames: String(l.get("assetGatewayGrayUsernames") || "").split(/\r?\n|,/).map((p) => p.trim()).filter(Boolean) }, hashPiAppBridge: { enabled: String(l.get("hashPiAppBridgeEnabled")) === "true", allowedModes: l.getAll("hashPiAppBridgeMode").filter(Boolean), ticketTtlSeconds: Number(l.get("hashPiAppBridgeTicketTtlSeconds") || 120), failureMessage: String(l.get("hashPiAppBridgeFailureMessage") || "HashPi APP 登录失败，请返回 HashPi 后重试。").trim(), piRechargeHint: String(l.get("hashPiAppBridgePiRechargeHint") || "APP 内暂不支持 Pi 充值，请用 Pi Browser 打开 Pi闪电战完成充值。").trim() }, timing: { quickBotFallbackSeconds: Number(l.get("quickBotFallbackSeconds") || 30), matchCancelWaitSeconds: Number(l.get("matchCancelWaitSeconds") || 20), matchCancelCooldownSeconds: Number(l.get("matchCancelCooldownSeconds") || 10), waitingReadyTimeoutSeconds: Number(l.get("waitingReadyTimeoutSeconds") || 30), vsIntroSeconds: Number(l.get("vsIntroSeconds") || 5), readyCountdownSeconds: Number(l.get("readyCountdownSeconds") || 6), quickRoundSeconds: Number(l.get("quickRoundSeconds") || 75), paidRoundSeconds: Number(l.get("paidRoundSeconds") || 90), botMoveIntervalSeconds: Number(l.get("botMoveIntervalSeconds") || 2.6) }, capacity: { maxActiveRooms: Number(l.get("maxActiveRooms") || 500), maxQueueLengthPerMode: Number(l.get("maxQueueLengthPerMode") || 2e3), realtimeMaxConnectionsPerInstance: Number(l.get("realtimeMaxConnectionsPerInstance") || 1200), realtimeMaxConnectionsPerUser: Number(l.get("realtimeMaxConnectionsPerUser") || 2), realtimeHeartbeatSeconds: Number(l.get("realtimeHeartbeatSeconds") || 25), realtimeIdleTimeoutSeconds: Number(l.get("realtimeIdleTimeoutSeconds") || 90), realtimeMaxPayloadBytes: Number(l.get("realtimeMaxPayloadBytes") || 2048) }, extremeRealtime: { enabled: true, rollbackToLegacy: false, enabledModes: ["quick_battle", "points_battle", "poc_battle", "pi_battle"], grayPercent: 100, grayUserPiUids: [], grayUserPiUsernames: [], maxPendingSwaps: Number(l.get("extremeRealtimeMaxPendingSwaps") || 3), snapshotIntervalMs: Number(l.get("extremeRealtimeSnapshotIntervalMs") || 2e3), swapMinIntervalMs: Number(l.get("extremeRealtimeSwapMinIntervalMs") || 120), metricsSampleRate: Number(l.get("extremeRealtimeMetricsSampleRate") || 0.05) }, withdrawRisk: a.withdrawRisk, rechargeBonus: a.rechargeBonus, visualEffects: { defaultMode: String(l.get("effectDefaultMode") || "balanced"), piBrowserDefaultMode: String(l.get("effectPiBrowserDefaultMode") || "balanced"), allowUserChoice: String(l.get("effectAllowUserChoice")) === "true", allowHighMode: true, autoDowngradeEnabled: String(l.get("effectAutoDowngradeEnabled")) === "true", dragTrailEnabled: String(l.get("effectDragTrailEnabled")) === "true", hapticEnabled: String(l.get("effectHapticEnabled")) === "true", soundEnabled: String(l.get("effectSoundEnabled")) === "true", soundVolume: Number(l.get("effectSoundVolume") || 0.9), bgmEnabled: String(l.get("effectBgmEnabled")) === "true", bgmVolume: Number(l.get("effectBgmVolume") || 0.28), attackWarningEnabled: String(l.get("effectAttackWarningEnabled")) === "true", attackWarningText: String(l.get("effectAttackWarningText") || "\u88AB\u653B\u51FB \u538B\u529B+{attack}").trim(), animationDurations: { localBurstSeconds: Number(l.get("effectLocalBurstSeconds") || 1.18), localBurstHighSeconds: Number(l.get("effectLocalBurstHighSeconds") || 1.26), serverBurstSeconds: Number(l.get("effectServerBurstSeconds") || 1.22), serverBurstHighSeconds: Number(l.get("effectServerBurstHighSeconds") || 1.28), lowPerformanceBurstSeconds: Number(l.get("effectLowPerformanceBurstSeconds") || 0.9), boardEffectSeconds: Number(l.get("effectBoardEffectSeconds") || 0.28), boardEffectHighSeconds: Number(l.get("effectBoardEffectHighSeconds") || 0.34), tileBurstSeconds: Number(l.get("effectTileBurstSeconds") || 0.34), tileBurstHighSeconds: Number(l.get("effectTileBurstHighSeconds") || 0.38), tileFallSeconds: Number(l.get("effectTileFallSeconds") || 0.26), tileFallHighSeconds: Number(l.get("effectTileFallHighSeconds") || 0.28), localSwapSeconds: Number(l.get("effectLocalSwapSeconds") || 0.18), invalidSwapSeconds: Number(l.get("effectInvalidSwapSeconds") || 0.2), serverSettleSeconds: Number(l.get("effectServerSettleSeconds") || 0.16), impactSeconds: Number(l.get("effectImpactSeconds") || 0.82), impactHighSeconds: Number(l.get("effectImpactHighSeconds") || 0.92), pressureHitSeconds: Number(l.get("effectPressureHitSeconds") || 0.72), boardUnderAttackSeconds: Number(l.get("effectBoardUnderAttackSeconds") || 0.58), attackLineSeconds: Number(l.get("effectAttackLineSeconds") || 0.78), hitWarningSeconds: Number(l.get("effectHitWarningSeconds") || 0.62) } } })) }), x && (x.textContent = "\u4FDD\u5B58\u6210\u529F\uFF0C\u6B63\u5728\u5237\u65B0..."), await q();
    } catch (p) {
      x && (x.textContent = g(p));
    }
